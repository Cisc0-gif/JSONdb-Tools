...
function exportAllSheets(e) {
  ...
  for (var i = 0; i < sheets.length; i++) {
    var sheet = sheets[i];
    var rowsData = getColsData_(sheet, getExportOptions(e));
...
// getColsData iterates row by row in the input range and returns an array of key:value pairs.
// Each object contains all the data for a given row, indexed by its normalized column name.
// Arguments:
//   - sheet: the sheet object that contains the data to be processed
//   - options: ExportOptions() field
// Returns an Array of key:value pairs
function getColsData_(sheet, options) {
  ...
  var objects = getColsObjects_(dataRange.getValues(), normalizeHeaders_(headersRange.getValues()));
  return objects;
  /*
  if (options.structure == STRUCTURE_HASH) {
    var objectsById = {};
...
function getColsObjects_(data, keys) {
  //var objects = [];
  var object = {};
  for (var i = 0; i < data.length; ++i) {
    var hasData = false;
